...
namespace ЭКЗ_Шарики
{
    public partial class Form1 : Form
    {
        ...
        {
            if (int.TryParse(txtbSpeed.Text, out int speed))
            {
                if (int.Parse(txtbSpeed.Text) > 100 || int.Parse(txtbSpeed.Text) <= 0)
                {
                    MessageBox.Show("Максимальная скороть 100");
                }
                else { timer1.Interval = (101 - int.Parse(txtbSpeed.Text)); }
            }
        }
    }
}
